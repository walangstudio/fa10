...
#[test]
fn restore_rejects_oversized_manifest_length() {
    let dir = tempfile::tempdir().unwrap();
    let archive = dir.path().join("evil.fa10");
    let mut bytes = Vec::new();
    bytes.extend_from_slice(HEADER_MAGIC);
    ...
    bytes.extend_from_slice(&u64::MAX.to_le_bytes()); // absurd manifest length
    fs::write(&archive, &bytes).unwrap();

    // Must error cleanly, not panic or attempt a giant allocation.
    let mut ropts = RestoreOptions::new(archive);
    ropts.output = Some(dir.path().join("out"));
    assert!(restore::restore(&ropts, &NoProgress).is_err());
}

#[test]
fn restore_rejects_huge_entry_count() {
    let dir = tempfile::tempdir().unwrap();
    let archive = dir.path().join("evil2.fa10");
    // 16-byte manifest body: magic + entry_count=u32::MAX + 4 junk (crc) bytes.
    let mut manifest = Vec::new();
    manifest.extend_from_slice(b"FA10MANI");
    manifest.extend_from_slice(&u32::MAX.to_le_bytes());
    manifest.extend_from_slice(&[0u8; 4]);
    let mut bytes = Vec::new();
    bytes.extend_from_slice(HEADER_MAGIC);
...
#[test]
fn restore_rejects_inflated_entry_size() {
    let dir = tempfile::tempdir().unwrap();
    let archive = dir.path().join("inflated.fa10");
    // One real file entry claiming far more content than the archive holds.
    let manifest = Manifest {
        entries: vec![Entry {
            kind: EntryKind::File,
            path: "f".to_string(),
    ...
    let mut bytes = Vec::new();
    bytes.extend_from_slice(HEADER_MAGIC);
    bytes.extend_from_slice(b"x"); // only 1 byte of content
    manifest.write_to(&mut bytes).unwrap();
    fs::write(&archive, &bytes).unwrap();

    let mut ropts = RestoreOptions::new(archive);
    ropts.output = Some(dir.path().join("out"));
    ropts.verify = false; // even without hashing, the size lie must be caught
    assert!(restore::restore(&ropts, &NoProgress).is_err());
}
